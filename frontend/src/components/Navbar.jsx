...
export default function Navbar() {
    ...
    return (
        <nav className={`flex items-center justify-between fixed z-50 top-0 w-full px-6 md:px-16 lg:px-24 xl:px-32 py-4 ${openMobileMenu ? '' : 'backdrop-blur'}`}>
            <Link href="/">
                <img className="h-9 md:h-9.5 w-auto shrink-0" src={theme === "dark" ? "https://media.istockphoto.com/id/1405050567/vector/health-stethoscope-vector-logo-design-stethoscope-with-tree-icon-vector-design.jpg?s=612x612&w=0&k=20&c=8mKYvw9I3mnuMW6V8LA2KHy9DRCuZdWCNy_UBpZZWXE=" : "/assets/logo-dark.svg"} alt="Logo" width={140} height={40} priority fetchPriority="high" />
            </Link>
            ...
            <div className="flex items-center gap-4">
                ...
                <button className="hidden md:block hover:bg-slate-100 dark:hover:bg-purple-950 transition px-4 py-2 border border-purple-600 rounded-md">
                   <Link to={isLoaded?"profile":"sign-in" }className="hover:text-slate-600 dark:hover:text-slate-300">
                       {isLoaded?user.firstName:"Sign in"}
                    </Link>
                </button>
